...
func NewEditCmd(_ io.Writer, _ []string) *cobra.Command {
	cmd := &cobra.Command{
		Use:          "edit [RELEASE]",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return errors.Wrapf(err, "create temporary file '%s' err", tmpfile.Name())
			}
			...
			if err != nil {
				return errors.Wrap(err, "get release values err")
			}

			if _, err := tmpfile.Write(currentValues); err != nil {
				return errors.Wrapf(err, "write values to temporary file '%s' err", tmpfile.Name())
			}

			if err := tmpfile.Close(); err != nil {
				return errors.Wrapf(err, "close temporary file '%s' err", tmpfile.Name())
			}

			editorVal := os.Getenv("EDITOR")
			var editor []string
			if len(editorVal) == 0 {
				// default editor
				editor = []string{"vi"}
			} else {
				editor = strings.Split(editorVal, " ")
			}
			...
			if err := command.Run(); err != nil {
				return errors.Wrapf(err, "use editor '%s' to open temporary file '%s' err", editor[0], tmpfile.Name())
			}

			newValues, err := ioutil.ReadFile(tmpfile.Name())
			if err != nil {
				return errors.Wrapf(err, "read temporary file '%s' err", tmpfile.Name())
			}

			if string(currentValues) != string(newValues) {
				if err := upgrade(cfg, args[0], tmpfile.Name()); err != nil {
					return err
				}
			}
...
func upgrade(cfg *action.Configuration, name, file string) (err error) {
	...
	if _, err := client.Run(rel.Name, rel.Chart, vals); err != nil {
